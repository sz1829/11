...
class AllOrderHistory {
    function getHistoryOrder() {
        ...
        if ($currency == 'USD') {
            ...
            $default_exchange_rate = $result->fetch_assoc()['value'];
        }
        $profit_min = empty($_GET['profit_min']) ? '-999999999.99' : $_GET['profit_min'] / $default_exchange_rate;
        $profit_max = empty($_GET['profit_max']) ? '9999999.99' : $_GET['profit_max'] / $default_exchange_rate;
        $cost_min = empty($_GET['cost_min']) ? '-999999999.99' : $_GET['cost_min'] / $default_exchange_rate;
        $cost_max = empty($_GET['cost_max']) ? '99999999.99' : $_GET['cost_max'] / $default_exchange_rate;
        $price_min = empty($_GET['price_min']) ? '-999999999.99' : $_GET['price_min'] / $default_exchange_rate;
        $price_max = empty($_GET['price_max']) ? '99999999.99' : $_GET['price_max'] / $default_exchange_rate;
        $sql_group = "SELECT
                  t.transaction_id, 
        ...
        $sql_indiv = "SELECT
            ...
            ((
                ...
                OR
                (t.currency = 'RMB'
                AND t.total_profit/(SELECT exchange_rate FROM IndividualTour WHERE indiv_tour_id = t.indiv_tour_id) >= $profit_min
                AND t.total_profit/(SELECT exchange_rate FROM IndividualTour WHERE indiv_tour_id = t.indiv_tour_id) <= $profit_max
                AND t.expense/(SELECT exchange_rate FROM IndividualTour WHERE indiv_tour_id = t.indiv_tour_id) >= $cost_min
                AND t.expense/(SELECT exchange_rate FROM IndividualTour WHERE indiv_tour_id = t.indiv_tour_id) <= $cost_max
                AND t.received/(SELECT exchange_rate FROM IndividualTour WHERE indiv_tour_id = t.indiv_tour_id) >= $price_min
                AND t.received/(SELECT exchange_rate FROM IndividualTour WHERE indiv_tour_id = t.indiv_tour_id) <= $price_max
                ))
            ";
        ...
        $sql = "SELECT * FROM (" . $sql_group . " UNION " . $sql_indiv . " UNION " . $sql_airticket. ") AS t WHERE 1";
        $sql .= $statusFilter;
