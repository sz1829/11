...
//导游姓名的宽度：
function guideNameCell() {
	var dlBox = $(".resultInfo").find("ul li.resultNav").find("dl");
	var liBox = $(".resultInfo").find("ul li.resultDetail").find("dl");
	var linshi = [];
	for(var i = 0; i < dlBox.find("dd").length; i++) {
		if($.trim($(".resultInfo ul li.resultNav").find("dd").eq(i).text()) == "导游姓名") {
			$(".resultInfo").find("ul li.resultDetail").each(function(j, item) {
				//			 	if($(item).find("dd").eq(i).width()>110){
				//			 		$(item).find("dd").eq(i).addClass("guideName");
				//			 		console.log(i);
				//			 	}
				$(item).find("dd").eq(i).css("width", localStorage.getItem("max_ddWidth"));
				linshi.push($(item).find("dd").eq(i).width());
				return linshi;
			});
		}
	}
	var max = linshi[0];
	for(var index = 0; index < linshi.length; index++) {
		if(linshi[index] > max) {
			max = linshi[index];
		}
	}
//	console.log("最大值：" + max);
	localStorage.setItem("max_ddWidth", max);
}
